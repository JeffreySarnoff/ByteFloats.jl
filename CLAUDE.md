...
The PDF ([docs/pdf/ByteFloats.jl-0.1.0.pdf](docs/pdf/ByteFloats.jl-0.1.0.pdf))
is regenerated by the measurement-driven pipeline in
[docs/pdf/generate_pdf.md](docs/pdf/generate_pdf.md) — resolve via
`make_latex.jl`, survey/transform the .tex, XeLaTeX, then render-and-inspect
every page. [docs/pdf/buildnote.md](docs/pdf/buildnote.md) records the last
build's toolchain, transforms, and verification evidence.
...
| `defaults.jl` | settable session defaults; `default_projspec` reads `DefaultProjection()`, so `a + b`, `Exp(x)`, and `T(x::Real)` follow it — consumed through a speculation guard that keeps them allocation-free at the initial default |
...
| `rand.jl` | Random-API hooks: `rand` (uniform [0,1) floor-projected), `randn` (normal, SatFinite, signed formats only); scalar `::Type` forms take a `projection` keyword, stochastic ρ draws from the caller's rng |
...
7. **Registry-driven codegen.** `OP_REGISTRY` in `ops_scalar.jl` is the single
   ...
   the registry should generate; that is the non-divergence mechanism. The Base
   register in `juliacompat.jl` is part of it: every op must be either mapped to
   a Base name or listed in `_NO_BASE_COUNTERPART` — the suite asserts that
   partition, so adding an op forces the decision.
...
`ternary_opt.jl` (ternary table tiers, sticky-head FMA/FAA escalation) and
`quality.jl` are both included from `runtests.jl` and run by `Pkg.test()`.
Two files are manual-only:
